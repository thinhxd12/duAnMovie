...
export default function BaiTapTongHop(props) {
    ...
    return (
        <div className="flex">
            <div className="h-full p-3 space-y-2 w-1/6 bg-coolGray-50 text-coolGray-800">
                <div className="flex items-center p-2 space-x-4">
                    <img src="https://picsum.photos/200/200" alt className="w-12 h-12 rounded-full" />
                    <div>
                ...
                <div className="divide-y divide-coolGray-300">
                    <ul className="pt-2 pb-4 space-y-1 text-sm">
                        {mangDanhMuc.map((danhMuc, index) => {
                            return <li className="bg-coolGray-100 text-coolGray-900" onClick={() => setMaDanhMuc(danhMuc.maDanhMuc)} key={index}>
                                <a href="#" className="flex items-center p-2 space-x-3 rounded-md">
                                    ...
                                    <span className="text-base font-semibold text-gray-900">{danhMuc.tenDanhMuc}</span>
                                </a>
                            </li>
                        })}
            ...
            <section className="text-gray-600 body-font w-5/6">
                <div className="container px-5 py-24 mx-auto">
                    <div className="flex flex-wrap -m-4">
                        {_.filter(data, monAn => monAn.maDanhMuc === maDanhMuc).map((monAn, index) => {
                            return <div className="p-4 lg:w-1/3" key={index}>
                                <div className="h-full bg-gray-100 bg-opacity-75 px-8 py-4 rounded-lg overflow-hidden text-center relative shadow-md">
                                    <h2 className="tracking-widest text-xs title-font font-medium text-gray-400 mb-1">{monAn.tenDanhMuc}</h2>
                                    <h1 className="title-font sm:text-2xl text-xl font-medium text-gray-900 mb-3 h-16">{monAn.tenMonAn}</h1>
                                    <img src={monAn.hinhAnh} alt="..." className="rounded-lg" />
                                    <a className="text-indigo-500 inline-flex items-center mt-4 text-base font-semibold" onClick={() => {
                                        dispatch({
                                            type: 'OPEN_MODAL',
                                            open:true,
                                            monAn: <div>
                                    ...
                                    <p className="text-base mt-2 text-right">Giá: ${monAn.gia}</p>
